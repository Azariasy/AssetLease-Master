  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (importType === 'ledger') {
        if (isExcel) rows = await parseExcelData(file);
        else rows = parseCSVData(await readFileAsText(file));
      } else {
        if (isExcel) rows = await parseExcelBalance(file);
        else rows = parseBalanceCSV(await readFileAsText(file));
      }
             ...
             <tbody className="divide-y divide-slate-100">
               {previewRows.map((row: any, idx) => (
                 <tr key={idx} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-2 text-xs text-slate-400">
                        {row.costCenter && <div title="成本中心">{row.costCenter}</div>}
